// 携程社招（custom）
// 接口同校招：POST https://careers.ctrip.com/api/hrrecruit/getJobAd
//   {"condition":{...,"category":1},"pager":{...}}（category=1 = 社招，2 = 校招）
// 详情路由（SPA hash）：careers.ctrip.com/#/experienced/job-detail/{fromId}（注意用 fromId，非 jobId UUID）。
const fs = require('fs');
const path = require('path');

const COMPANY = '携程集团';
const KEY = 'ctrip_social';
...
async function fetchPage(index) {
  const body = {
    condition: { fromId: [], keyword: '', kind: [], country: [], city: [], bucode: [], jobFamilyCode: [], jobFamilyGroupCode: [], category: 1 },
  ...
  for (let index = 1; index <= 30; index++) {
    ...
    for (const d of list) {
      ...
      jobs.push({
        ...
        url: d.fromId ? (BASE + '/#/experienced/job-detail/' + d.fromId) : '',
