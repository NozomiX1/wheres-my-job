// 上海人工智能实验室社招：GET https://www.shlab.org.cn/api/getJobList?mode=social
// page_token 翻页（limit 上限 10）。社招池约 120+，研究员/青年科学家岗密集。
const fs = require('fs');
const path = require('path');

const COMPANY = '上海AI实验室';
const KEY = 'shlab_social';
...
async function getList(page_token, limit) {
  const u = `${BASE}?mode=social&jobFunction=&location=&jobType=&subject=&keyword=&page_token=${encodeURIComponent(page_token || '')}&limit=${limit}`;
  const r = await fetch(u, {
    headers: {
      'User-Agent': UA,
      'Referer': 'https://www.shlab.org.cn/joinus/social',
  ...
  for (let i = 0; i < 100; i++) {
    ...
    for (const it of items) {
      ...
      jobs.push({
        ...
        url: it.id ? ('https://www.shlab.org.cn/joinus/detail/' + it.id + '?mode=social') : '',
        desc: [it.description, it.requirement].filter(Boolean).join('\n'),
        commitment: zhName(it.job_recruitment_type) || '社招',
        id: String(it.id || '')
      });
    }
