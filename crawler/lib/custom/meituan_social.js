...
// jobShareType=2（社招）。约 2000+ 岗，pageSize=100 翻页拉全量。
const fs = require('fs');
const path = require('path');

const COMPANY = '美团';
const KEY = 'meituan_social';
...
async function fetchAll() {
  ...
  while (true) {
    const body = {
      ...
      jobShareType: '2',           // 1=校招 2=社招
      keywords: '',
      cityList: [], department: [], jfJgList: [],
      jobType: [], typeCode: [], specialCode: [],
      u_query_id: 'x', r_query_id: String(Date.now()),
    };
    const r = await fetch(URL, { method: 'POST', headers: H, body: JSON.stringify(body) });
    const j = await r.json();
    if (!j || !j.data || !Array.isArray(j.data.list)) throw new Error('meituan social fail: ' + JSON.stringify(j).slice(0, 200));
    ...
    for (const x of list) {
      ...
      jobs.push({
        ...
        dept: dept || '-',
        // jobFamily=官方职类（技术类/职能类/销售客服与支持类…）
        category: String(x.jobFamily || ''),
        city: city || '-',
        date: fmtDate(x.firstPostTime),
        url: 'https://zhaopin.meituan.com/web/position/detail?jobUnionId=' + x.jobUnionId + '&highlightType=social',
